...
1.  What are the dimensions of the Denny's dataset?
    ...
    ANSWER: 1643 x 6. Each row represent the franchise location. Variables are address, city, state, zip, longtitude and latitude.

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
    ANSWER: 999x6. Each row also represent the franchise location. Variables are address, city, state, zip, longtitude and latitude.
...
3.  Take a look at the websites that the data come from (linked above).
    ...
    ANSWER: 
    La Quinta's locations: Canada, Mexica, China, New Zealand, Georgia, Turkiye, United of Arab Emirates, Columbia, Ecuador
    Denny's locations:Canada, Mexico, and Puerto Rico,Japan, Philippines,New Zealand, UK, Guatemala, El Salvador, Curaçao, Honduras, UAE
...
4.  Now take a look at the data.
    ...
    ANSWER: If we have the locations of all the La Quinta and Denny in a dataset. Then filter out locations not from the US, we will           retreived an dataset with all the locations outside of US. The code should be something like this. 
...
```
    ANSWER: There is a tibble of 0x6 which mean there is no franchise of Denny outside US
...
9.  Which states have the most and fewest Denny's locations? What about La Quinta? Is this surprising? Why or why not?
    ...
    ANSWER: 
    For Dennys, DE has the fewest Dennys location and CA has the most franchise. 
    For Laquinta, TX has the most location and ME has the least location. 
    It suprising that Laquinta has most location in TX not CA. 
...
11. Filter the data for observations in North Carolina only, and recreate the plot.
    ...
    ANSWER: It seem like Mitch Hedberg's joke appear to be true visually. Most of the Laquinta location was a few miles next to Dennys. However there are a significant amount of Dennys location compared to the Laquinta. It seem like the hostability industry is not well developed. 
```{r eval= TRUE}
dn_lq %>%
  filter(state=="NC")%>%
  ggplot(aes(x = longitude, y = latitude, color = establishment))+
    ...
    labs(
      title = " Location of Denny and LaQuinta in North Carolina",
      x = "Longtitude",
      y = "Latitude"
    ) +
    theme_minimal()
...
12. Now filter the data for observations in Texas only, and recreate the plot, with an appropriate `alpha` level.
    Visually, does Mitch Hedberg's joke appear to hold here?
      ...
      ANSWER: It seem like Mitch Hedberg's joke also appear to be true visually. Opposite of North Carolina, there are a massive amount of Laquinta hotel than Dennys restaurant in Texas. However, the joke still be true where most of the Dennys restaurant seem to be open right next to the Laquinta hotel. 
...
dn_lq %>%
  ...
  ggplot(aes(x = longitude, y = latitude, color = establishment))+
    ...
    labs(
      color = "Establishment:",
      title = " Location of Denny and LaQuinta in Texas",
      x = "Longtitude",
      y = "Latitude"
    ) +
    theme_minimal()
